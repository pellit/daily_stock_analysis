...
@router.post(
    ...
    description="Run a backtest evaluation over historical analysis records and persist results to backtest_results/backtest_summaries.",
)
def run_backtest(
...
@router.get(
    ...
    description="Paginated backtest results with optional stock code filter.",
)
def get_backtest_results(
    code: Optional[str] = Query(None, description="Filter by stock code"),
    eval_window_days: Optional[int] = Query(None, ge=1, le=120, description="Filter by evaluation window (days)"),
    analysis_date_from: Optional[date] = Query(None, description="Analysis start date (inclusive)"),
    analysis_date_to: Optional[date] = Query(None, description="Analysis end date (inclusive)"),
...
def get_stock_performance(
    ...
    try:
        ...
        if summary is None:
            raise HTTPException(
                status_code=404,
                detail={"error": "not_found", "message": f"No backtest summary found for {code}"},
            )
        return PerformanceMetrics(**summary)
    except ValueError as exc:
        raise HTTPException(
