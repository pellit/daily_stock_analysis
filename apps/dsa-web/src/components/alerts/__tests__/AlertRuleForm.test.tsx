...
describe('AlertRuleForm', () => {
  ...
  it('submits a price_cross rule payload', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Rule name'), { target: { value: 'Moutai price breakout' } });
    fireEvent.change(screen.getByLabelText('Symbol code'), { target: { value: '600519' } });
  ...
  it('submits a volume_spike rule payload and supports disabled creation', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Symbol code'), { target: { value: 'msft' } });
    fireEvent.change(screen.getByLabelText('Rule type'), { target: { value: 'volume_spike' } });
    fireEvent.change(screen.getByLabelText('Volume spike multiplier'), { target: { value: '2.5' } });
    fireEvent.click(screen.getByLabelText('Enable on creation'));
    fireEvent.click(screen.getByRole('button', { name: 'Create rule' }));
  ...
  it('submits technical indicator rule payloads', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Crossover direction'), { target: { value: 'bearish_cross' } });
  ...
  it('rejects indicator period combinations that exceed fetchable history', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('MACD period combination requires 501 daily candles; 365 max');
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it('rejects invalid stock code format before submit', () => {
    ...
    fireEvent.change(screen.getByLabelText('Symbol code'), { target: { value: 'aapl-2026' } });
    fireEvent.change(screen.getByLabelText('Price threshold'), { target: { value: '200' } });
    fireEvent.click(screen.getByRole('button', { name: 'Create rule' }));

    expect(screen.getByRole('alert')).toHaveTextContent('Invalid symbol code format');
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it('shows JP/KR options for market region in Chinese UI mode', () => {
    ...
    expect(screen.getByRole('option', { name: 'A-share (cn)' })).toBeInTheDocument();
