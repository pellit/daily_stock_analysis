...
describe('LLMChannelEditor', () => {
  ...
  it('reports unsaved channel edits as generation backend draft items', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Model (逗号min隔) '), {
  ...
  it('preserves an invalid saved API surface during an unrelated save', async () => {
    ...
    expect(screen.getByRole('option', { name: 'invalidConfig: respones' })).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText('Base URL'), {
      target: { value: 'https://proxy.example.com/v1' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Save AI Config' }));
  ...
  it('does not emit invalid channel env keys while the channel name is empty', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /OpenAI 官方/i }));
    fireEvent.change(await screen.findByLabelText('ChannelName'), { target: { value: '' } });
  ...
  it('shows help dialogs for channel editor fields', async () => {
    ...
    expect(screen.getByText('该Channel Interface根address.')).toBeInTheDocument();
    ...
    expect(screen.getByText('Runhour统一采sample温度.')).toBeInTheDocument();

    fireEvent.keyDown(document, { key: 'Escape' });
    fireEvent.click(await screen.findByRole('button', { name: 'View RunhourCapabilityDetect Configuration help' }));

    expect(screen.getByRole('dialog', { name: 'RunhourCapabilityDetect' })).toBeInTheDocument();
    expect(screen.getByText('SelectCapabilityafter点击Detect; DetectwillSendstart真实 LLM Request.')).toBeInTheDocument();
  });
  ...
  it('excludes Hermes-only route from Agent and Vision runtime selects', () => {
    ...
    expect(selectOptionValues('main Model')).toContain('openai/hermes-agent');
    expect(selectOptionValues('Agent main Model')).not.toContain('openai/hermes-agent');
  ...
  it('keeps mixed Hermes route for Agent but excludes it from Vision', () => {
    ...
    expect(selectOptionValues('main Model')).not.toContain('openai/shared-route');
    expect(selectOptionValues('main Model')).toContain('openai/pure-route');
    expect(selectOptionValues('Agent main Model')).toContain('openai/shared-route');
  ...
  it('keeps pure non-Hermes route in Agent and Vision runtime selects', () => {
    ...
    expect(selectOptionValues('main Model')).toContain('openai/gpt-4o-mini');
    expect(selectOptionValues('Agent main Model')).toContain('openai/gpt-4o-mini');
  ...
  it('keeps minimax-prefixed models in runtime selections', () => {
    ...
    const primaryModelSelect = screen.getByRole('combobox', { name: 'main Model' });
    const agentModelSelect = screen.getByRole('combobox', { name: 'Agent main Model' });
  ...
  it('uses DeepSeek V4 defaults when adding the official preset', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '+ AddChannel' }));
    ...
    expect(screen.getByLabelText('Model (逗号min隔) ')).toHaveValue('deepseek-v4-flash,deepseek-v4-pro');
  });
  ...
  it.each([
    ...
    expect(screen.getByLabelText('Model (逗号min隔) ')).toHaveValue(models);
  });
  ...
  it('shows provider capability badges, official sources, and config hints', async () => {
    ...
    expect(await screen.findByText('ConfigReference')).toBeInTheDocument();
    expect(screen.getByText('OpenAI compatible')).toBeInTheDocument();
    expect(screen.getByText('aggregate平units')).toBeInTheDocument();
    expect(screen.getByText('canGetModel')).toBeInTheDocument();
    expect(screen.getByText(/ModelList和Model可见性依赖账号Permission与 API Key/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'OpenRouter Models API' })).toHaveAttribute(
      'href',
      'https://openrouter.ai/docs/api/api-reference/models/get-models',
    );
    expect(screen.getByText(/能力Tag仅用于Config参考，不代表运行时能力已验证通过/i)).toBeInTheDocument();
  });
  ...
  it('shows model-discovery capability for SiliconFlow provider hints', async () => {
    ...
    expect(await screen.findByText('canGetModel')).toBeInTheDocument();
  ...
  it('does not show provider metadata for custom or unknown channels', async () => {
    ...
    expect(screen.queryByText('ConfigReference')).not.toBeInTheDocument();
    expect(screen.queryByText(/官方Source/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/能力Tag仅用于Config参考/i)).not.toBeInTheDocument();
  });
  ...
  it('preserves manually edited base URL and models when switching preset names', async () => {
    ...
    fireEvent.change(screen.getByLabelText('ChannelName'), {
    ...
    expect(screen.getByLabelText('Model (逗号min隔) ')).toHaveValue('custom-model-a,custom-model-b');
  });
  ...
  it('uses the selected preset defaults when adding a duplicate provider channel', async () => {
    ...
    expect(screen.getAllByLabelText('ChannelName').map((input) => (input as HTMLInputElement).value)).toEqual([
      'minimax',
      'minimax2',
    ]);
    ...
    expect(screen.getAllByLabelText('Model (逗号min隔) ').map((input) => (input as HTMLInputElement).value)).toEqual([
  ...
  it('clears active Hermes unsupported multi-key and extra-header env keys on save', async () => {
    update.mockResolvedValue({
      ...
      warnings: [
        'Detected removal of Hermes Phase 3 unsupported config items: LLM_HERMES_API_KEYS, LLM_HERMES_EXTRA_HEADERS. The Hermes reserved channel supports only a single LLM_HERMES_API_KEY; multi-key or extra headers are rejected. To restore old values, manually recover from .env backup, git history, or desktop export. Non-empty LLM_HERMES_API_KEYS / LLM_HERMES_EXTRA_HEADERS will still be rejected by backend validation.',
      ],
    });
    ...
    fireEvent.click(screen.getByRole('button', { name: /Hermes/i }));
    fireEvent.change(screen.getByLabelText('Model (逗号min隔) '), { target: { value: 'hermes-agent,hermes-agent-2' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save AI Config' }));
    ...
    expect(await screen.findByText(/已清理 Hermes Phase 3 不支持的Config项/i)).toBeInTheDocument();
    expect(screen.getByText(/如需Restore旧值，请从 \.env Backup/i)).toBeInTheDocument();
  });
  ...
  it('only persists edited values for runtime-only channel keys', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /my_proxy/i }));
    fireEvent.change(screen.getByLabelText('Model (逗号min隔) '), { target: { value: 'gpt-4o-mini,gpt-4o' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save AI Config' }));
  ...
  it('renames a mixed raw/runtime channel and clears persisted API key field', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /my_proxy/i }));
    fireEvent.change(screen.getByLabelText('ChannelName'), { target: { value: 'my_proxy2' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save AI Config' }));
  ...
  it('prompts when bare runtime models loosely match canonical OpenAI route aliases', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('The current runtime model uses a non-canonical route alias. Pick a canonical model from the dropdown.')).toBeInTheDocument();
    });
  ...
  it('shows cleanup warning and restore path after stale runtime models are removed on save', async () => {
    update.mockResolvedValue({
      ...
      updatedKeys: ['LLM_DEEPSEEK_MODELS', 'LITELLM_MODEL'],
      warnings: [
        'Detected removal of stale runtime model references in primary / agent primary / vision / fallback models. To restore, repopulate the channel model list and reselect; or use the desktop export backup or manually restore previous LLM_* / LITELLM_MODEL / AGENT_LITELLM_MODEL / VISION_MODEL / LLM_TEMPERATURE values from .env.',
      ],
    });
    ...
    expect(await screen.findByText('Saveafterinfo')).toBeInTheDocument();
    expect(screen.getByText(/已同步清理失效的运行时Model引用/i)).toBeInTheDocument();
    expect(screen.getByText(/桌面端ExportBackup或Manual \.env Revert/i)).toBeInTheDocument();
  });

  it('keeps save warnings visible after onSaved-driven refresh', async () => {
    const warningMessage = 'Detected removal of stale runtime model references in primary / agent primary / vision / fallback models.';
    const initialItems = [
      { key: 'LLM_CHANNELS', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_PROTOCOL', value: 'deepseek' },
      { key: 'LLM_DEEPSEEK_BASE_URL', value: 'https://api.deepseek.com' },
  ...
  it('keeps runtime selections while channel models are edited temporarily', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /DeepSeek 官方/i }));
    const modelInput = screen.getByLabelText('Model (逗号min隔) ');
  ...
  it('checks protocol-prefixed selected model when discovery returns bare id', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('ManualModel (逗号min隔) ')).toHaveValue('');
    });
  });
  ...
  it('does not treat unknown-prefixed selected model as equivalent to bare discovered id', async () => {
    ...
    expect(screen.getByLabelText('ManualModel (逗号min隔) ')).toHaveValue('minimax/MiniMax-M1');
  });
  ...
  it('discovers models and writes selected values back to channel config', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('ManualModel (逗号min隔) ')).toHaveValue('qwen-old,qwen-plus');
    });
  ...
  it('shows structured troubleshooting hint when channel auth fails', async () => {
    ...
    expect(await screen.findByText(/聊dayInvoke · 鉴权Failure：LLM authentication failed/i)).toBeInTheDocument();
    expect(screen.getByText(/请Check API Key Whether正确/i)).toBeInTheDocument();
  ...
  it('does not show model-list action hints for network failures', async () => {
    ...
    expect(await screen.findByText(/域名Parse failed/i)).toBeInTheDocument();
    expect(screen.queryByText(/调整Model顺序或Remove不AvailableModel/i)).not.toBeInTheDocument();
  });
  ...
  it('does not apply stale discovery response after channel list re-sync', async () => {
    ...
    expect(screen.getByLabelText('ManualModel (逗号min隔) ')).toHaveValue('dash-old,dashscope-plus');
  ...
  it('does not apply stale discovery response after inline channel edit', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Model (逗号min隔) ')).toHaveValue('qwen-old');
      expect(screen.queryByLabelText('stale-openai')).not.toBeInTheDocument();
    });
  });
});
