...
const API_SURFACE_OPTIONS: Array<{ value: LLMApiSurface; label: string }> = [
  { value: 'chat_completions', label: 'Chat Completions (default)' },
...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  { value: 'json', label: 'JSON', hint: 'Check whether response_format JSON output is available.' },
  { value: 'tools', label: 'Tools', hint: 'Check whether function/tool calling is available.' },
  { value: 'stream', label: 'Stream', hint: 'Check whether streaming output returns valid chunks.' },
  { value: 'vision', label: 'Vision', hint: 'Check whether the current model accepts image_url input.' },
];
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = 'The runtime-injected Hermes Key is not echoed back; to test it on the settings page, re-enter the Key or save it to .env.';
...
const ChannelRow: React.FC<ChannelRowProps> = ({
  ...
  const apiSurfaceOptions = API_SURFACE_OPTIONS.some((option) => option.value === channel.apiSurface)
    ? API_SURFACE_OPTIONS
    : [
        { value: channel.apiSurface, label: `Invalid config: ${channel.apiSurface}` },
        ...API_SURFACE_OPTIONS,
      ];
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                placeholder="Select API Surface"
              />
            </div>
          </div>
          ...
          <Input
            ...
            placeholder={
              channel.protocol === 'gemini' || channel.protocol === 'anthropic'
                ? 'Official endpoint can be left blank'
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] font-medium text-muted-text">Config reference</span>
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                Capability tags are for config reference only and do not imply that runtime capabilities have been verified.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? 'Local Ollama can be left blank' : 'Supports multiple Keys separated by commas'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? 'If custom model names are not in the list, you can still add them manually; saved format remains comma-separated.'
                  : 'If the channel does not support auto-discovery or the request fails, you can fill in the model list directly.'
              }
            />
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    The basic connection test uses the first item in the model list by default: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  Real LLM requests are only sent when manually triggered; selecting multiple can take 20-40 seconds.
                </p>
              </div>
              <Button
                type="button"
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'Check that the API Key is correct, has no extra spaces, and that the channel requires no extra org/project permissions.',
  timeout: 'You can retry; if it keeps timing out, check the Base URL, network proxy, provider region, or local firewall.',
  quota: 'Check your balance, plan quota, RPM/TPM rate limits, or concurrency settings, and try again later if needed.',
  model_not_found: 'Confirm the model name matches the channel protocol and use "Fetch models" to verify the actual available model list.',
  empty_response: 'The channel is reachable but returned no body; try a different compatible model or disable extra response modes before retesting.',
  network_error: 'Check the Base URL, proxy, TLS/certificates, relay gateway, or local network policy, and try again later.',
  invalid_config: 'Fill in the protocol, Base URL, API Key, and model configuration first, then run the one-click test.',
  unsupported_protocol: 'Auto model discovery is currently only provided for OpenAI Compatible / DeepSeek channels; please maintain the model list manually.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API Key is empty, or after comma-splitting no usable Key remains; enter at least one valid Key before testing.',
  api_key_rejected: 'The provider rejected the current API Key; check the Key, org/project permissions, region, and account status.',
  rate_limit: 'The provider triggered RPM/TPM or concurrency rate limits; reduce request frequency or try again later.',
  insufficient_balance: 'The provider reports insufficient balance, billing, or quota; check your account balance and plan status.',
  quota_exceeded: 'The provider reports the quota is exhausted; confirm your account plan, remaining balance, and project quota.',
  provider_blocked: 'The request was blocked by the provider or relay gateway; check account risk controls, regional restrictions, model permissions, reseller gateway policies, content safety policies, or request source restrictions.',
  ...
  model_access_denied: 'The current account cannot use this model; confirm whether the model is enabled, visible to the account, or has been disabled.',
  provider_prefix_mismatch: 'The model provider prefix does not match the current channel; confirm whether the model name should use this channel\'s OpenAI-compatible route.',
  capability_unsupported: 'The current model or compatibility layer does not support this capability; this does not affect basic text connectivity, so you may switch models or remove dependencies on this capability.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? 'This channel returned an incompatible /models response format; please fill in the model list manually.'
      : 'The response structure does not match expectations; please confirm that this channel is compatible with the Chat Completions interface.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return 'This channel\'s /models endpoint did not return any usable model IDs; check whether the Base URL points to a compatible model listing endpoint, or fill in the model list manually.';
  }
...
function buildLlmTestHint(result: {
  ...
  const modelHint = testedModel ? `Model tested in this run: ${testedModel}.` : '';
  const scopeInfo = 'The basic connection test only tests the first model in the model list by default.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? 'If this model is unavailable, adjust the model order or remove the unavailable model and try again.'
    : '';
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel name cannot be empty and may only contain letters, digits, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes routes are not yet supported as primary or fallback models; please select a pure Hermes or pure non-Hermes route.' });
        return;
      }
      ...
      if (nonCanonicalRouteAlias) {
        setSaveMessage({ type: 'local-error', text: 'The current runtime model uses a non-canonical route alias; please reselect the canonical model from the dropdown.' });
        return;
      }
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'The current primary model is not in the model list of any enabled channel; please select again.' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'The current Agent primary model has no Agent-safe non-Hermes deployment; please select again.' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'There is an invalid fallback model; please select again.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'The current Vision model cannot include a Hermes deployment; please select a pure non-Hermes route.' });
        return;
      }
    }
    ...
    try {
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: result.success ? 'success' : 'error',
          text: result.success
            ? `Fetched ${result.models.length} model${result.models.length === 1 ? '' : 's'}${result.latencyMs ? ` · ${result.latencyMs} ms` : ''}`
            : buildLlmFailureText(result),
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            After adding a provider channel, you can automatically fetch the model list and select multiple; you may also continue to fill them in manually. Configuration is automatically synced to the .env file.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">Select a preset provider first, then create a config draft with one click.</p>
              </div>
          ...
          <div className="space-y-2">
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-text">Select a provider preset and then click "Add channel" to start configuring.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-text">The primary model, fallback models, Vision, and Temperature are written directly to the runtime configuration.</p>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  Controls model output randomness: 0 is deterministic, 2 is maximum randomness, 0.7 is recommended.
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  First add at least one enabled channel and fill in the models; the primary / fallback / Vision options below will then appear.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Auto (inherit the regular analysis primary model)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-secondary-text">
                      Fallback models are only used when the primary model fails. The primary model is not added to the fallback models again.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Auto (follow the Vision default logic)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
            ...
            <InlineAlert
              variant="warning"
              message="A custom model routing YAML has been detected: only channel entries and basic connection info are managed here. The runtime primary model / fallback models / Vision / Temperature are still decided by the generic fields below; if the YAML parses successfully, its routing and available-model declarations take precedence, and this configuration will not overwrite the YAML file itself."
