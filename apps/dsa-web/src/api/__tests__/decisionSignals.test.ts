...
describe('decisionSignalsApi', () => {
  ...
  it('creates signals with top-level field mapping and opaque JSON pass-through', async () => {
    post.mockResolvedValueOnce({
      data: {
        item: {
          ...
          stop_loss: 1600,
          target_price: 1850,
          invalidation: 'breakdownSupport',
          watch_conditions: 'volumeBreakout',
          reason: 'Trendimproving',
          risk_summary: 'volatility较high',
          catalyst_summary: 'Sectorrepair',
    ...
    const response = await decisionSignalsApi.create({
      ...
      targetPrice: 1850,
      invalidation: 'breakdownSupport',
      watchConditions: 'volumeBreakout',
      reason: 'Trendimproving',
      riskSummary: 'volatility较high',
      catalystSummary: 'Sectorrepair',
  ...
  it('persists reassess and parses the authoritative server item', async () => {
    post.mockResolvedValueOnce({
      data: {
        ...
        warnings: [{ code: 'action_adjusted_by_guardrail', message: 'Adjusted.' }],
        blocked_reason: null,
      },
    });
