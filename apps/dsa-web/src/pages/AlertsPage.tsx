...
const AlertsPage: React.FC = () => {
  ...
  const renderTestResultMessage = (result: AlertRuleTestResponse): React.ReactNode => {
    const targetResults = result.targetResults ?? [];
    const observed = result.observedValue == null ? '--' : String(result.observedValue);
    const yesNo = result.triggered ? t('alerts.testResultYes') : t('alerts.testResultNo');
    return (
      <div className="space-y-2">
        <div>
          {t('alerts.testResultMessage', {
  ...
  const notificationChannelLabel: Record<string, string> = {
    __cooldown__: t('alerts.notificationChannel.cooldown'),
    __cooldown_read_failed__: t('alerts.notificationChannel.cooldownReadFailed'),
    __noise_suppressed__: t('alerts.notificationChannel.noiseSuppressed'),
    __no_channel__: t('alerts.notificationChannel.noChannel'),
    __dispatch__: t('alerts.notificationChannel.dispatch'),
    __context__: t('alerts.notificationChannel.context'),
  };

  const formatNotificationChannel = (channel: string): string =>
    notificationChannelLabel[channel] ?? channel;

  const formatNotificationStatus = (notification: AlertNotificationItem): string => {
    if (notification.success) return t('alerts.notificationStatus.success');
    if (notification.errorCode === 'cooldown_active') return t('alerts.notificationStatus.cooldownActive');
    if (notification.errorCode === 'cooldown_read_failed') return t('alerts.notificationStatus.cooldownReadFailed');
    if (notification.errorCode === 'noise_suppressed') return t('alerts.notificationStatus.noiseSuppressed');
    if (notification.errorCode === 'no_channel') return t('alerts.notificationStatus.noChannel');
    return t('alerts.notificationStatus.failure');
  };

  return (
    <AppPage className="space-y-5">
      <PageHeader
        eyebrow={t('alerts.pageEyebrow')}
        title={t('alerts.pageHeading')}
        description={t('alerts.pageDescription')}
      />
      ...
      <div className="grid items-stretch gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        <div className="flex h-full min-h-0 flex-col gap-4">
          ...
          {testResult ? (
            <InlineAlert
              title={t('alerts.testResultTitle')}
      ...
      <Card title={t('alerts.notifications.title')} subtitle={t('alerts.notifications.subtitle')} variant="bordered" padding="md">
        {notificationsLoading ? <Loading label={t('alerts.notifications.loading')} /> : null}
        {!notificationsLoading && notifications.length === 0 ? (
          <EmptyState
            icon={<BellRing className="h-6 w-6" />}
            title={t('alerts.notifications.emptyTitle')}
            description={t('alerts.notifications.emptyDescription')}
          />
        ) : null}
        {!notificationsLoading && notifications.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[680px] text-left text-sm">
              <thead className="border-b border-border/60 text-xs uppercase text-muted-text">
                <tr>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.channel')}</th>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.status')}</th>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.errorCode')}</th>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.latency')}</th>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.time')}</th>
                  <th className="px-3 py-2 font-medium">{t('alerts.notifications.column.diagnostics')}</th>
                </tr>
              </thead>
