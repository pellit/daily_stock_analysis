...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning(text.brokerLoadWarningEmpty);
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning(text.brokerLoadWarningFailed);
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || text.riskWarningFailed);
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(formatUiText(text.positionAnalysisSubmitted, { symbol: row.symbol, taskId: task.taskId }));
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning(text.writeWarningSelectAccountForEntry);
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning(text.writeWarningSelectAccountForDelete);
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning(text.writeWarningSelectAccountForPositionDelete);
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError(text.accountCreateNameRequired);
    ...
    try {
      ...
      setAccountCreateSuccess(text.accountCreateSuccess);
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || text.accountCreateError);
      setAccountCreateSuccess(null);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">{text.accountCreateTitle}</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              >
                {text.accountCreateCollapse}
              </button>
            ) : (
              <span className="text-xs text-secondary">{text.accountCreateHint}</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            <input
              ...
              placeholder={text.accountCreateNamePlaceholder}
              value={accountForm.name}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, name: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={text.accountCreateBrokerPlaceholder}
              value={accountForm.broker}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, broker: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={text.accountCreateBaseCurrencyPlaceholder}
            ...
            >
              <option value="cn">{text.accountCreateMarketCn}</option>
              <option value="hk">{text.accountCreateMarketHk}</option>
              <option value="us">{text.accountCreateMarketUs}</option>
              <option value="jp">{text.accountCreateMarketJp}</option>
              <option value="kr">{text.accountCreateMarketKr}</option>
              <option value="tw">{text.accountCreateMarketTw}</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? text.accountCreateCreating : text.accountCreateCreate}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.formTradeTitle}</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.formTradeSymbolPlaceholder} value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={tradeForm.side}
                ...
                <option value="buy">{text.formTradeBuy}</option>
                <option value="sell">{text.formTradeSell}</option>
              </select>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.formTradeQuantityPlaceholder} value={tradeForm.quantity}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, quantity: e.target.value }))} required />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.formTradePricePlaceholder} value={tradeForm.price}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, price: e.target.value }))} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.formTradeFeePlaceholder} value={tradeForm.fee}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, fee: e.target.value }))} />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.formTradeTaxPlaceholder} value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">{text.formTradeFeeHint}</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.formTradeSubmit}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.formCashTitle}</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">{text.formCashIn}</option>
                <option value="out">{text.formCashOut}</option>
              </select>
            </div>
            <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={text.formCashAmountPlaceholder}
              value={cashForm.amount} onChange={(e) => setCashForm((prev) => ({ ...prev, amount: e.target.value }))} required />
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={formatUiText(text.formCashCurrencyPlaceholder, { currency: writableAccount?.baseCurrency || text.formCashDefaultCurrency })} value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))} />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.formCashSubmit}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.formCorpTitle}</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.formCorpSymbolPlaceholder} value={corpForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="cash_dividend">{text.formCorpDividend}</option>
                <option value="split_adjustment">{text.formCorpSplit}</option>
              </select>
            </div>
            {corpForm.actionType === 'cash_dividend' ? (
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={text.formCorpDividendPlaceholder}
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={text.formCorpSplitPlaceholder}
                value={corpForm.splitRatio}
                onChange={(e) => setCorpForm((prev) => ({ ...prev, splitRatio: e.target.value, cashDividendPerShare: '' }))} required />
            )}
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{text.formCorpSubmit}</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.formCsvTitle}</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  ...
                  <option value="huatai">{text.formCsvBrokerHuatai}</option>
                )}
              </select>
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">{text.formCsvDryRun}</label>
            </div>
            <div className="flex gap-2">
              <button type="button" className="btn-secondary flex-1" disabled={!csvFile || csvParsing} onClick={() => void handleParseCsv()}>
                {csvParsing ? text.formCsvParsing : text.formCsvParseFile}
              </button>
              <button type="button" className="btn-secondary flex-1"
                disabled={!csvFile || !writableAccountId || csvCommitting} onClick={() => void handleCommitCsv()}>
                {csvCommitting ? text.formCsvCommitting : text.formCsvCommitImport}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                variant={getCsvParseVariant(csvParseResult)}
                title={text.formCsvParseResultTitle}
                message={formatUiText(text.formCsvParseResultMessage, {
            ...
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? text.formCsvCommitResultTitleDry : text.formCsvCommitResultTitleNormal}
                message={formatUiText(text.formCsvCommitResultMessage, {
                  label: csvDryRun ? text.csvCommitDryRunLabel : text.csvCommitActualLabel,
                  inserted: csvCommitResult.insertedCount,
                  duplicate: csvCommitResult.duplicateCount,
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{text.eventsTitle}</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">{text.eventsTypeTrade}</option>
                <option value="cash">{text.eventsTypeCash}</option>
                <option value="corporate">{text.eventsTypeCorporate}</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? text.eventsLoading : text.eventsRefresh}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder={text.eventsSymbolFilter} value={eventSymbol}
            ...
            {eventType === 'trade' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventSide} onChange={(e) => setEventSide(e.target.value as '' | PortfolioSide)}>
                <option value="">{text.eventsAllSides}</option>
                <option value="buy">{text.formTradeBuy}</option>
                <option value="sell">{text.formTradeSell}</option>
              </select>
            ) : null}
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">{text.eventsAllDirections}</option>
                <option value="in">{text.formCashIn}</option>
                <option value="out">{text.formCashOut}</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">{text.eventsAllActions}</option>
                <option value="cash_dividend">{text.formCorpDividend}</option>
                <option value="split_adjustment">{text.formCorpSplit}</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? text.eventsWriteBlockedHint : text.eventsWriteAllowedHint}
            </div>
              ...
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {formatUiText(text.eventsTradeLine, {
                      date: item.tradeDate,
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: formatUiText(text.eventsConfirmDeleteTrade, {
                          date: item.tradeDate,
                          side: formatSideLabel(item.side, language),
                          symbol: item.symbol,
                          quantity: item.quantity,
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: formatUiText(text.eventsConfirmDeleteCash, {
                          date: item.eventDate,
                          direction: formatCashDirectionLabel(item.direction, language),
                          amount: item.amount,
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: formatUiText(text.eventsConfirmDeleteCorp, {
                          date: item.effectiveDate,
                          action: formatCorporateActionLabel(item.actionType, language),
                          symbol: item.symbol,
                        }),
                      })}
                    >
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title={text.eventsEmptyTitle}
                      description={text.eventsEmptyDescription}
                      className="border-none bg-transparent px-3 py-6 shadow-none"
                    />
                  ) : null}
            </div>
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>{formatUiText(text.eventsPageIndicator, { page: eventPage, total: totalEventPages })}</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        title={text.eventsDeleteTitle}
        message={pendingDelete?.message || text.eventsDeleteMessage}
        confirmText={deleteLoading ? text.eventsDeleting : text.eventsDeleteConfirm}
        cancelText={text.eventsCancel}
        isDanger
