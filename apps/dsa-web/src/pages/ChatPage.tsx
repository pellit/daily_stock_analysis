...
import {
  CHOICE_COMPARE_STOCK_MESSAGE_RE,
  LINKED_COMPARE_STOCK_MESSAGE_RE,
  STRONG_COMPARE_STOCK_MESSAGE_RE,
  SWITCH_STOCK_MESSAGE_RE,
...
const QUICK_QUESTIONS: Array<{
  ...
  { label: 'Analyze Maotai with Chan theory', skill: 'chan_theory', stockContext: { stock_code: '600519', stock_name: 'Kweichow Maotai' } },
  { label: 'Wave theory view on CATL', skill: 'wave_theory', stockContext: { stock_code: '300750', stock_name: 'CATL' } },
  { label: 'Analyze BYD trend', skill: 'bull_trend', stockContext: { stock_code: '002594', stock_name: 'BYD' } },
  { label: 'Box oscillation analysis on SMIC 688981', skill: 'box_oscillation', stockContext: { stock_code: '688981', stock_name: 'SMIC' } },
  { label: 'Analyze Tencent hk00700', skill: 'bull_trend', stockContext: { stock_code: 'HK00700', stock_name: 'Tencent' } },
  { label: 'Emotion cycle analysis on East Money', skill: 'emotion_cycle', stockContext: { stock_code: '300059', stock_name: 'East Money' } },
];
...
const ChatPage: React.FC = () => {
  ...
  // Get localized report text (follows active UI language)
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(t('chat.watchlist.removed', { code: stockCode }));
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(t('chat.watchlist.added', { code: stockCode }));
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage(t('chat.watchlist.actionFailed'));
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || t('chat.contextCompression.loadFailed'));
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || t('chat.contextCompression.saveFailed'));
      } finally {
  ...
  // Handle follow-up from report page: ?stock=600519&name=<stockName>&recordId=xxx
  useEffect(() => {
  ...
  const handleSend = useCallback(
    async (
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : [t('chat.skills.general')];
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user'
      ? t('chat.downloadHeading.user')
      : t('chat.downloadHeading.assistant', { skill: skillLabel || '' });
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = t('chat.thinking.summary', { count: toolSteps.length, seconds: totalDuration.toFixed(1) });
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">{t('chat.thinking.label')}</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          ...
          text = getPipelineBudgetSkippedLabel(step, t);
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          <DashboardStateBlock
            loading
            compact
            title={t('chat.sidebar.loading')}
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : sessions.length === 0 ? (
          <DashboardStateBlock
            compact
            title={t('chat.sidebar.emptyTitle')}
            description={t('chat.sidebar.emptyDescription')}
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={t('chat.session.switchAria', { title: s.title })}
                ...
                >
                  ...
                  <div className="content">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="meta">
                        {t('chat.session.messageCount', { count: s.message_count })}
                      </span>
                      {s.last_active && (
                        <>
                ...
                <button
                  ...
                  aria-label={t('chat.session.deleteAria', { title: s.title })}
                >
                  <svg
          ...
          <div className="flex items-start justify-between gap-4">
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              <button
                ...
                aria-label={t('chat.sidebar.menuAria')}
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                ...
                <Tooltip content={t('chat.sendToChannel')}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: t('chat.sendSuccess') }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || t('chat.sendFailedTitle'),
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label={t('chat.sendToChannel')}
                    >
                      {sending ? (
                        <svg
                  ...
                  >
                    {msg.role === 'assistant' && (skillLabel || msg.backend) && (
                      <div className="mb-2 flex flex-wrap gap-2">
                        {skillLabel ? <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={t('chat.skills.skillAria', { label: skillLabel })}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
            ...
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  ...
                  message={t('chat.followUp.message')}
                  className="rounded-xl px-3 py-2 text-xs shadow-none"
                />
              ) : null}
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  />
                  <span className="font-medium">{t('chat.contextCompression.title')}</span>
                  <span className="text-xs text-muted-text">{t('chat.contextCompression.hint')}</span>
                </label>
                <span className="text-xs text-muted-text">
                  {contextCompressionSaving
                    ? t('chat.contextCompression.saving')
                    : contextCompressionEnabled
                      ? t('chat.featureEnabled')
                      : t('chat.featureDisabled')}
                </span>
              </div>
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title={t('chat.contextCompression.saveFailedTitle')}
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  <button
                    ...
                    aria-label={mobileSkillPickerOpen ? t('chat.skills.collapseAria') : t('chat.skills.expandAria')}
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="flex-shrink-0 font-medium">{t('chat.skills.label')}</span>
            ...
            {activeStockCode && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-[11px]"
                >
                  {stockInWatchlist(activeStockCode) ? t('chat.watchlist.removeButton') : t('chat.watchlist.addButton')}
                </Button>
                {watchlistMessage && (
